...
# actually the source directory
SOURCE=$(pwd)

cpuInfo() {
	sysctl hw.model | awk '{ $1 = ""; print "CPU model: " $0 }' > $TMP
	...
	while true; do
		...
		if [ $count -ne 0 ]; then
			...
			dialog	--title "CPU INFO" --gauge "$(cat $CPU)" 17 60
		else
			echo 0 | dialog --title "CPU INFO" --gauge "$(cat $CPU)" 17 60
		fi
			
		cat $TMP > $CPU
		top -Pd2 | grep ^CPU | tail -n2 | awk '{ print $1$2" USER: "$3" SYST: "$7" IDLE: "$11 }' >> $CPU
		
		# count lines = number of CPUs
...
fileBrowser() 
{
	...
	if [ $result -eq $DIALOG_OK ]; then
		dir=$(cat $FILE | grep $option -w | awk '{ if($2 ~ /^inode/) print }') # $dir will not be empty if the object is a inode
		#echo $dir
		#echo $option	
		if [[ ! -z $dir ]]; then
			fileBrowser $option
...
fileBrowser_info() 
{
	...
	text=$(cat $FILE | grep $option -w | awk '{ if($2 ~ /text/) print }')
	if [[ -z $text ]]; then
		dialog --title "File Browser" --msgbox "$(cat $TMP)" 17 50
	else
		dialog --title "File Browser" --yes-label "Ok" --no-label "Edit" --yesno "$(cat $TMP)" 17 50 
	fi
